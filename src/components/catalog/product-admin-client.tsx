  ...
  const filtered = useMemo(
    () =>
      initialProducts.filter((item) => {
        ...
        return (
          (!term || `${item.name} ${item.sku} ${item.model}`.toLowerCase().includes(term)) &&
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-base-ink/55">Kelola paket, harga, stok, dan model</p>
          </div>
        </div>
        <Button variant="sky" disabled={!categories.length} onClick={() => show(null)}>
      ...
      <div className="grid gap-2 lg:grid-cols-[1fr_220px_180px]">
        <label className="flex h-11 items-center gap-2 rounded-neo border-2 border-base-ink bg-white px-3 shadow-neo-sm">
          ...
          <input
            ...
            placeholder="Cari nama, SKU, model..."
        ...
        {filtered.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              <p className="relative mt-4 line-clamp-3 min-h-14 text-sm font-semibold leading-relaxed text-base-ink/65">
        ...
        <form action={save} className="space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-bold">SKU / kode</label>
              {stockMode === "external" ? (
                <select
                  name="sku"
