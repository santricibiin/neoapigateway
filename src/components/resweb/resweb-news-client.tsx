    ...
    <div className="space-y-6">
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-neo border border-base-line bg-accent-lavender p-5 shadow-neo sm:p-7">
        ...
        <div className="relative flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          <div><span className="inline-flex items-center gap-2 rounded-full border border-base-line bg-white px-3 py-1 text-[10px] font-black uppercase tracking-widest"><Radio className="h-3 w-3" /> Siaran Member</span><h1 className="mt-3 text-3xl font-black sm:text-4xl">Berita reseller Anda.</h1><p className="mt-1 text-sm font-bold text-base-ink/60">Hanya tampil untuk member yang Anda buat.</p></div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {initialNews.map((item, index) => (
          <motion.article key={item.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} whileHover={{ y: -4 }} className="relative overflow-hidden rounded-neo border border-base-line bg-white p-5 shadow-neo-sm">
            ...
            <div className="relative"><span className={`inline-flex rounded-full border border-base-line px-2.5 py-1 text-[10px] font-black uppercase ${item.active ? "bg-accent-mint" : "bg-base-bg"}`}>{item.active ? "Aktif" : "Draft"}</span><h2 className="mt-3 text-xl font-black">{item.title}</h2><p className="mt-2 line-clamp-5 whitespace-pre-wrap text-sm font-semibold leading-relaxed text-base-ink/65">{item.content}</p><p className="mt-4 text-[10px] font-bold uppercase tracking-wide text-base-ink/40">{new Date(item.updatedAt).toLocaleString("id-ID")}</p><div className="mt-4 flex gap-2 border-t border-base-line pt-4"><Button type="button" size="sm" variant="outline" onClick={() => show(item)}><FilePenLine className="h-4 w-4" /> Edit</Button><Button type="button" size="sm" className="bg-accent-terraSoft" onClick={() => void remove(item)}><Trash2 className="h-4 w-4" /> Hapus</Button></div></div>
          </motion.article>
        ))}
      </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Edit Berita" : "Tambah Berita"} className="max-h-[90vh] overflow-y-auto">
        <form action={save} className="space-y-4">
          ...
          <label className="block text-sm font-bold">Isi berita<textarea name="content" defaultValue={editing?.content || ""} minLength={3} maxLength={5000} rows={8} className="mt-1.5 w-full resize-y rounded-neo border border-base-line bg-white px-4 py-3 text-sm font-semibold shadow-neo-sm outline-none" required /></label>
