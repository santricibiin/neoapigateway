...
export function DashboardSidebar({
  ...
  return (
    <motion.aside
      ...
      className="fixed left-0 top-16 bottom-0 z-40 my-3 ml-3 hidden flex-col overflow-hidden rounded-neo border border-base-line bg-base-ink shadow-neo-lg lg:flex"
    >
        ...
        <AnimatePresence>
          {!collapsed ? (
            <motion.div
              ...
              className="min-w-0"
            >
              <p className="truncate text-sm font-extrabold text-white">{siteName}</p>
              <p className="text-[9px] font-bold uppercase tracking-widest text-white/40">Admin Panel</p>
            </motion.div>
          ) : null}
        </AnimatePresence>
      </div>
