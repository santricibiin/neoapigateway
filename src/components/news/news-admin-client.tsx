  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-neo border border-base-line bg-accent-lavender shadow-neo-sm"><Megaphone className="h-5 w-5" /></span><div><h1 className="text-2xl font-black">Berita Member</h1><p className="text-sm font-semibold text-base-ink/55">Atur pesan popup di dashboard member</p></div></div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {initialNews.map((item, index) => (
          <motion.article key={item.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }} className="relative overflow-hidden rounded-neo border border-base-line bg-base-surface p-5 shadow-neo-sm">
            ...
            <div className="relative mt-4 flex flex-wrap gap-2 border-t border-base-line pt-4">
              ...
              <Button type="button" size="sm" className="bg-accent-terraSoft text-base-ink" onClick={() => void remove(item)}><Trash2 className="h-4 w-4" /> Hapus</Button>
            </div>
          </motion.article>
        ))}
      </div>
